...
histPlot(obs, xlab="Male heights (in)", ylab='', axes=FALSE, main='', xlim=c(60,80), ylim=c(0,.15), probability=TRUE, col=COL[1])
axis(1)
M  <- mean(obs)
SD <- sd(obs)
...
par(mar=c(3,4,1,0))
qqnorm(obs, cex=0.5, main='', axes=F, ylab='Male heights (in)', pch=19, col=COL[1,2])
axis(1)
axis(2)
